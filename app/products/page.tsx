...
export default function FoodItemsPage() {
  ...
  const handleOrder = async (foodItemId: string) => {
    try {
      const response = await apiFetch("/api/orders", {
        method: "POST",
        body: JSON.stringify({ productId: foodItemId, quantity: 1 }),
      })
      ...
      if (response.ok) {
        toast({
          variant: "success",
          title: "Order Placed!",
          description: "Your order has been placed successfully. Waiting for admin approval.",
        })
      } else {
        const data = await response.json()
        toast({
                ...
                {filteredItems.map((item) => (
                  ...
                  >
                    ...
                    <CardContent className="space-y-4">
                      ...
                      {/* Order Button */}
                      <Button
                        onClick={() => handleOrder(item.id || item._id || "")}
